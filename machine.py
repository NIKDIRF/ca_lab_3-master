...
class ControlUnit:
    ...
    def instr_mov(self):
        # Чтение первого аргумента команды
        ...
        self.tick()
        logging.debug('%s', self)
        # Следующая по порядку инструкция
    ...
    def instr_cmp_rel_inc(self):
        # Чтение первого аргумента команды
        self.data_path.latch_data_addr(self.args[0])
        self.data_path.latch_acc(AccMuxSignals.MEM)
        self.tick()
        logging.debug('%s', self)
        # Чтение из памяти по адресу в аккумуляторе
    ...
    def instr_in(self):
        # Запись в память из входного буфера
    ...
    def instr_out_char(self):
        # Чтение первого аргумента команды
        self.data_path.latch_data_addr(self.args[0])
        self.data_path.latch_acc(AccMuxSignals.MEM)
        self.tick()
        logging.debug('%s', self)
        # Сигнал вывода символа
    ...
    def instr_jmp(self):
        # Переход по памяти команд
        self.program_counter = self.args[0]
        self.tick()
        logging.debug('%s', self)

    def instr_je(self):
        # Переход по памяти команд при флаге zero
    ...
    def instr_sv(self):
        # Запись в память по адресу первого аргумента
        self.data_path.latch_data_addr(self.args[0])
        self.data_path.write(self.data_path.acc)
        self.tick()
        logging.debug('%s', self)
    ...
    instructions = {
        ...
        'mov': instr_mov,
        'cmp': instr_cmp,
        'cmp_rel_inc': instr_cmp_rel_inc,
        'rdiv': instr_rdiv,
        'add': instr_add,
        'in': instr_in,
        'out': instr_out,
        'out_char': instr_out_char,
        'out_rel': instr_out_rel,
        'jmp': instr_jmp,
        'je': instr_je,
        'sv': instr_sv
    }
